...
class SimulationParams:
    ...
    potential_type: str = 'linear'  # 'linear' (F2) or 'log' (Coulomb)
    d_min: float = 0.1             # Regularization for log potential
    ...
    init_type: str = 'uniform'      # 'uniform' or 'gaussian' (Big Bang)
    init_gauss_variance: float = 0.01  # tangent-plane std for Big Bang IC
...
class BrownianParticlesS2:
    ...
    def __init__(self, params: SimulationParams):
        ...
        if getattr(params, 'init_type', 'uniform') == 'gaussian':
            self.positions = self._gaussian_blob_on_S2(
                self.N, getattr(params, 'init_gauss_variance', 0.01))
        else:
    ...
    def _gaussian_blob_on_S2(self, n: int, variance: float) -> np.ndarray:
        """Big Bang initial condition: n particles drawn from a Gaussian
        blob of standard deviation `variance` on the tangent plane to a
        randomly chosen center direction, then projected to S^2. Matches
        the HTML simulator init when initType='gaussian'."""
        center = np.random.randn(3)
        center /= np.linalg.norm(center)
        ref = np.array([1.0, 0.0, 0.0]) if abs(center[0]) < 0.9 \
            else np.array([0.0, 1.0, 0.0])
        t1 = ref - (ref @ center) * center
        t1 /= np.linalg.norm(t1)
        t2 = np.cross(center, t1)
        u = np.random.randn(n) * variance
        v = np.random.randn(n) * variance
        pts = center[None, :] + u[:, None] * t1[None, :] \
            + v[:, None] * t2[None, :]
        norms = np.linalg.norm(pts, axis=1, keepdims=True)
        return self.R * pts / norms
    ...
    def compute_forces(self) -> np.ndarray:
        """Compute forces on all particles (fully vectorized, no Python loop)."""
        pos = self.positions  # (N, 3)
        N = self.N
        R2 = self.R ** 2

        # All pairwise dot products: dot_ij = x_i · x_j
        dots = pos @ pos.T  # (N, N)

        # Geodesic distances (not needed for direction, but for validity check)
        cos_angles = np.clip(dots / R2, -1.0, 1.0)

        # Tangent vector from i toward j in tangent space at i:
        # t_ij = x_j - (x_i · x_j / R^2) x_i, then normalize
        # proj_factor[i,j] = dot_ij / R^2
        proj_factor = dots / R2  # (N, N)

        # proj_xj[i, j, :] = pos[j, :] - proj_factor[i, j] * pos[i, :]
        # = pos[j] - (x_i · x_j / R^2) x_i
        proj = pos[np.newaxis, :, :] - proj_factor[:, :, np.newaxis] * pos[:, np.newaxis, :]  # (N, N, 3)

        # Norms of projections
        proj_norms = np.linalg.norm(proj, axis=2)  # (N, N)

        # Avoid division by zero (diagonal and antipodal pairs)
        valid = proj_norms > 1e-10
        safe_norms = np.where(valid, proj_norms, 1.0)

        # Unit tangent vectors
        tangent = proj / safe_norms[:, :, np.newaxis]  # (N, N, 3)
        tangent[~valid] = 0.0

        # F_i = sum_j phi_ij * dV/dd * tangent_ij
        # For linear: dV/dd = 1
        # For log: dV/dd = 1/(d + d_min)
        if self.potential_type == 'log':
            distances = self.R * np.arccos(cos_angles)
            force_scale = self.phi / (distances + self.d_min)
            force_scale[~valid] = 0.0
        else:
            force_scale = self.phi  # linear: constant

        forces = np.einsum('ij,ijk->ik', force_scale, tangent)  # (N, 3)
